...
class WizardBancoAtlas(models.TransientModel):
    _name = 'wizard_bancos_atlas'
    _description = 'Wizard Banco ATLAS'
    ...
    def get_values_for_report_bancos_atlas(self):
        if not self.env.user.company_id.banco_atlas_codigo_empresa or not self.env.user.company_id.banco_atlas_nro_cuenta:
        ...
        ]).filtered(lambda x: x.employee_id.bank_id.is_banco_atlas)

        final_text = ''
        total_control = 0

        for payslip in payslips:
            total_amount = sum(line.amount for line in payslip.line_ids if line.category_id.code == 'NET' or 'NETO')
            ...
            if int(total_amount) > 0:  # Convertir a entero para validar
                ...
                record = self.get_formatted_string_right(payslip.employee_id.bank_account, 10) + \
        ...
        control_record = self.get_formatted_string_right(self.env.user.company_id.banco_atlas_nro_cuenta, 10) + \
    ...
    def print_wizard_bancos_atlas_report(self):
        action = self.env.ref('rrhh_payroll_bancos_atlas.wizard_bancos_atlas_report').report_action(self)
        return action