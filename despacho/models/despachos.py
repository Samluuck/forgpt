...
class Despacho(models.Model):
    ...
    total_oficializacion = fields.Float(
        string='Total de Oficialización',
        compute='_compute_total_oficializacion',
        store=True
    )

    @api.depends('documentos_oficializacion.monto')
    def _compute_total_oficializacion(self):
        for record in self:
            record.total_oficializacion = sum(record.documentos_oficializacion.mapped('monto'))
    ...
    factura_cliente_id = fields.Many2one('account.move', string='Factura Cliente')
