...
class HrCustody(models.Model):
    ...
    read_only = fields.Boolean(string="Campo de verificación")
    documento_apoyo = fields.Binary(string="Documento de Apoyo de la Custodia", attachment=True)
    permiso_admin = fields.Boolean(string="Permiso Administrativo")

    hr_manager = fields.Many2one('hr.employee', string='Encargado de RRHH')
    ...
    def print_payslip(self):
        # Llamar al informe desde aquí
        return self.env.ref('hr.custody.report_grupo_universal_document_template').report_action(self)
    ...
    def renew_refuse(self):
        for custody in self.env['hr.custody'].search([('custody_name', '=', self.custody_name.id)]):
            if custody.state == "approved":
                raise UserError(_("La custodia no está disponible ahora"))
        self.renew_date = ''
        self.state = 'approved'
    ...
    def validate_return_date(self):
        if self.return_date < self.date_request:
            raise ValidationError(_('Indique una fecha de devolución válida'))

    name = fields.Char(string='Code', copy=False, help="Code")
    company_id = fields.Many2one('res.company', 'Company', readonly=True, help="Company",
                                 default=lambda self: self.env.user.company_id)
    rejected_reason = fields.Text(string='Motivo del rechazo', copy=False, readonly=1, help="Reason for the rejection")
    renew_rejected_reason = fields.Text(string='Razón de rechazo de renovación', copy=False, readonly=1,
                                        help="Renew rejected reason")
    date_request = fields.Date(string='Fecha solicitada', required=True, track_visibility='always', readonly=True,
                               help="Fecha solicitada",
                               states={'draft': [('readonly', False)]}, default=datetime.now().strftime('%Y-%m-%d'))
    employee = fields.Many2one('hr.employee', string='Empleado', required=True, readonly=True, help="Employee",
                               default=lambda self: self.env.user.employee_id.id,
                               states={'draft': [('readonly', False)]})
    purpose = fields.Char(string='Razón', track_visibility='always', required=True, readonly=True, help="Reason",
                          states={'draft': [('readonly', False)]})
    custody_name = fields.Many2one('custody.property', string='Propiedad', required=True, readonly=True,
                                   help="Property name",
                                   states={'draft': [('readonly', False)]}
                                   )
    return_date = fields.Date(string='Fecha de regreso', required=True, track_visibility='always', readonly=True,
                              help="Return date",
                              states={'draft': [('readonly', False)]})
    renew_date = fields.Date(string='Fecha de devolución de renovación', track_visibility='always',
    ...
    state = fields.Selection([('draft', 'Borrador'), ('to_approve', 'A la espera de la aprobación'), ('approved', 'Aprobada'),
                              ('returned', 'Devuelto'), ('rejected', 'Rechazado')], string='Estado', default='draft',
                             track_visibility='always')
    mail_send = fields.Boolean(string="Enviar correo")
...
class HrPropertyName(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', help="Company",
                                 default=lambda self: self.env.user.company_id)
    property_selection = fields.Selection([('empty', 'Sin conexión'),
                                           ('product', 'productos')],
                                          default='empty',
                                          string='Propiedad desde', help="Select the property")

    product_id = fields.Many2one('product.product', string='Producto', help="Product")
